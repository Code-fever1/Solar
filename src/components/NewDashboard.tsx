...
export const NewDashboard = memo(function NewDashboard() {
  ...
  const SNAP_BOTTOM = 0;
  const SNAP_TOP = Math.round(height * 0.35);
  useEffect(() => {
    scrollRef.current?.scrollTo({ y: SNAP_TOP, animated: false });
  }, [SNAP_TOP]);
  ...
  const heroBg = useMemo(() => {
    ...
    const isDayTime = sunriseMs && sunsetMs ? (Date.now() >= sunriseMs && Date.now() < sunsetMs) : weather.isDay;
    if (!isDayTime) return require("../../assets/images/z-night.jpeg");
    if (code >= 51 && code <= 67) return require("../../assets/images/z-rain-light.jpeg");
    if (code >= 80 && code <= 86) return require("../../assets/images/z-rain-light.jpeg");
    if (code >= 1 && code <= 3) return require("../../assets/images/z-clouds-dark.jpeg");
    if (code >= 45) return require("../../assets/images/z-fog.jpeg");
    return require("../../assets/images/z-fullsun.jpeg");
  }, [weather.code, weather.sunrise, weather.sunset, weather.isDay]);

  return <View style={styles.screen}><Image source={heroBg} style={{ position: "absolute", top: 0, left: 0, width, height }} resizeMode="stretch" /><LinearGradient colors={["rgba(0,0,0,0.25)", "rgba(0,0,0,0.1)", "rgba(0,0,0,0.4)"]} locations={[0, 0.35, 1]} style={{ position: "absolute", top: 0, left: 0, width, height }} /><View style={{ position: "absolute", top: insets.top + 6, left: 13, right: 13 }} pointerEvents="box-none"><View style={styles.header}><View><Text style={[styles.greeting, { color: theme.text }]}>{greeting}, Alijah</Text><Text style={[styles.location, { color: theme.textSecondary }]}>{time}</Text></View><View style={styles.headerRight}><View style={styles.bell}><Bell size={19} color={theme.text} /><View style={styles.notification} /></View></View></View></View><ScrollView ref={scrollRef} style={{ backgroundColor: "transparent" }} contentContainerStyle={[styles.content, { paddingTop: height * 0.85 }]} showsVerticalScrollIndicator={false} removeClippedSubviews={true} nestedScrollEnabled={true} scrollEventThrottle={16} bounces={false} decelerationRate="fast" snapToOffsets={[SNAP_BOTTOM, SNAP_TOP]} snapToStart snapToEnd onScroll={handleScroll}>
    <View style={{ width: "100%", borderTopLeftRadius: 28, borderTopRightRadius: 28, overflow: "hidden", minHeight: height * 0.65 }}>
      <LinearGradient colors={["rgba(255,255,255,0.55)", "rgba(255,255,255,0.85)", "rgba(255,255,255,1)"]} locations={[0, 0.15, 0.4]} style={StyleSheet.absoluteFill} />
