...
const FlowChart = memo(function FlowChart({ points, width, startOfToday }: { points: FlowPoint[]; width: number; startOfToday: number }) {
  const height = 140;
  ...
  return (
    ...
    >
      <Svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        ...
        {yLabels.map((yl, i) => (
          <G key={`y-${i}`}>
            <Line x1={chartLeft} y1={yl.y} x2={width - 12} y2={yl.y} stroke="rgba(142,167,196,0.08)" />
            <SvgText x={2} y={yl.y + 3} fill="#5A6B7E" fontSize="7" fontFamily="Outfit">{yl.kw.toFixed(1)}kW</SvgText>
          </G>
        ))}
        {/* Current time marker — vertical dashed line */}
        <Line x1={paths.currentX} y1="24" x2={paths.currentX} y2="104" stroke="rgba(255,255,255,0.15)" strokeWidth="1" strokeDasharray="2 3" />
        <Circle cx={paths.currentX} cy="22" r="2.5" fill="#F4F8FC" />
        {/* Energy flow lines */}
        ...
        {tooltip && (
          <>
            <Line x1={tooltip.x} y1="24" x2={tooltip.x} y2="104" stroke="rgba(255,255,255,0.3)" strokeWidth="1" />
      ...
      {tooltip && (
        ...
        >
          <Text style={fcStyles.tooltipTime}>{tooltip.time}</Text>
...
const fcStyles = StyleSheet.create({
  tooltip: {
    ...
    paddingVertical: 4,
    borderWidth: 1,
    borderColor: "rgba(176,199,224,0.15)",
    minWidth: 116,
  },
  tooltipTime: { color: "#F4F8FC", fontFamily: "Outfit", fontSize: 8, fontWeight: "700", marginBottom: 2 },
